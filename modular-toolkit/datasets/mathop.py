...
class AddModCNNData:
    def __init__(self, p, frac_train=0.1, seed=42):
        self.p = p
        self.frac_train = frac_train
        self.seed = seed
        self.data = self.math_operation()
        self.train, self.valid = self.split_data()

    def math_operation(self):
        random.seed(self.seed)
        return [(a, b, (a + b) % self.p) for a in range(self.p) for b in range(self.p)]

    def split_data(self):
        random.shuffle(self.data)
        n_total = len(self.data)
        n_val = int(n_total * self.frac_train)
        return self.data[n_val:], self.data[:n_val]